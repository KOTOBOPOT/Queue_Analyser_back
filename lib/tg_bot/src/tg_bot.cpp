...
SimpleBot::SimpleBot(std::string token)
  ...
  _bot.getEvents().onCommand("start", [&](TgBot::Message::Ptr message) {
    this->_bot.getApi().sendMessage(message->chat->id, "Бот запущен", false, 0,
                                    _keyboard);
  });
  ...
  _bot.getEvents().onAnyMessage([&](TgBot::Message::Ptr message) {
    ...
    while (it != this->_handlers.end() && !(*it)->isHandler(message)) it++;
...
void SimpleBot::addKeyboardButton(IBotHandler::Ptr handler, size_t row,
  ...
  if (_keyboard->keyboard.size() < row) _keyboard->keyboard.resize(row);
...
void SimpleBot::addMenuButton(IBotHandler::Ptr handler) {
  _bot.getEvents().onCommand(handler->getHandlerString(),
...
void SimpleBot::run() {
  std::cout << "Bot username: " << this->_bot.getApi().getMe()->username
            << std::endl;
